...
export default function TalentPoolPage() {
    ...
    const loadDropdownData = async () => {
      try {
        const [deptsData, posData] = await Promise.all([
          organizationStructureApi.departments.getAllDepartments({
            isActive: true,
          }),
          organizationStructureApi.positions.getAllPositions({
            isActive: true,
          }),
        ]);
  ...
  const loadCandidates = useCallback(async () => {
    try {
      ...
      if (status === 401) {
        ...
        errorMessage =
          "Permission denied. You need HR Employee, HR Manager, System Admin, or Recruiter role.";
      } else if (status === 404) {
        errorMessage =
          "Endpoint not found. Please check if the backend is running.";
      } else if (status === 500) {
  ...
  const handleViewResume = async (candidate: Candidate) => {
    try {
      ...
      try {
        console.log(
          `📄 Attempting to download resume for candidate: ${candidate._id}`
        );
        const blob = await recruitmentApi.downloadCandidateResume(
          candidate._id
        );
        ...
        if (!newWindow) {
          ...
          showToast(
            "Popup blocked. Please allow popups to view the resume.",
            "error"
          );
        ...
        console.log(
          "Primary resume endpoint failed, trying fallback methods...",
          apiError
        );
      }
      ...
      if (candidate.resumeUrl) {
        ...
        if (resumeUrl.startsWith("/") || resumeUrl.includes("uploads")) {
          ...
          const fullUrl = `${baseUrl}${
            resumeUrl.startsWith("/") ? resumeUrl : "/" + resumeUrl
          }`;
        ...
        if (/^[0-9a-fA-F]{24}$/.test(resumeUrl)) {
          try {
            ...
            throw new Error(
              "Failed to retrieve document. It may have been deleted or is inaccessible."
            );
          }
        }
      }
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Browse and manage all candidates in the organization's talent
                pool
              </p>
            </div>
          </div>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-sm text-gray-400 mb-4">
                This could mean there are no candidates registered in the system
                yet.
              </p>
              <div className="text-xs text-gray-400 space-y-1">
                ...
                <p>
                  • Ensure you have the correct permissions (HR Employee, HR
                  Manager, System Admin, or Recruiter)
                </p>
              </div>
            </CardContent>
          </Card>
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="flex gap-2 pt-4 border-t">
                      ...
                      <Button
                        ...
                        className="flex-1"
                      >
                        {loadingApplications[candidate._id]
                          ? "Loading..."
                          : "Applications"}
                      </Button>
                      <Button
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {selectedCandidate.personalEmail || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-500">Phone</p>
                  <p className="text-sm text-gray-900">
                    {selectedCandidate.mobilePhone || "N/A"}
                  </p>
                </div>
                  ...
                  <CardContent className="pt-4">
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                      ...
                      {(app.stage || app.currentStage) && (
                        <span>
                          Stage:{" "}
                          {(app.stage || app.currentStage || "").replace(
                            /_/g,
                            " "
                          )}
                        </span>
